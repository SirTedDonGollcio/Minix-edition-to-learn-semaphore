 ...
 */
  ...
  if (--sched_ticks == 0) {
	...
	if(rdy_head[USER_Q]->p_type == 1)
	{
		sched_ticks = 1*SCHED_RATE;
	}
	else if(rdy_head[USER_Q]->p_type == 2)
	{
		sched_ticks = 5*SCHED_RATE;
	}
	else if(rdy_head[USER_Q]->p_type == 3)
	{
		sched_ticks = 10*SCHED_RATE;
	}
	else
	{
