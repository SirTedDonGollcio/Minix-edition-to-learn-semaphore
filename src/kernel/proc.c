 ...
 */
  ...
  struct proc *tmp1, *tmp2;
  unsigned int type, nextT;
  
  if (rdy_head[USER_Q] == NIL_PROC) return;

  
  
  type = rdy_head[USER_Q]->p_type;
  ...
  while(( rdy_head[USER_Q]->p_nextready!=NIL_PROC)&&(rdy_head[USER_Q]->p_type != nextT))
  {
  	tmp1=rdy_head[USER_Q];
  	while(( tmp1->p_nextready!=NIL_PROC)&&(tmp1->p_nextready->p_type != nextT))
  	{
  		tmp1= tmp1->p_nextready;
  	}
  	if(tmp1->p_nextready != NIL_PROC)
  	{
  		tmp2 = tmp1->p_nextready;
  		tmp1->p_nextready = tmp2->p_nextready;
  		
  		if(tmp1->p_nextready == NIL_PROC)
	  	{
	  		rdy_tail[USER_Q] = tmp1;
	  	}
	  	tmp2->p_nextready = rdy_head[USER_Q];
	  	rdy_head[USER_Q] = tmp2;
	  	break;
  	}
  	else
  	{
